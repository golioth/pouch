 ...
 * virtio-rpmsg through remoteproc and a resource table. Zephyr's
 * DT-instantiable ipc_service backends speak their own static-vring layout,
 * which the Linux kernel's virtio_rpmsg_bus does not understand, so on these
 * systems the vdev must come from the resource table, exactly like the
...
static int pouch_ept_cb(struct rpmsg_endpoint *ept,
                        void *data,
                        size_t len,
                        uint32_t src,
                        void *priv)
