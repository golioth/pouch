...
#define SPI_BROKER_INIT_GPIO(inst)                                                                 \
    do                                                                                             \
    {                                                                                              \
        struct spi_broker_ctx *ctx = &spi_broker_ctx_##inst;                                       \
        if (ctx->has_data_ready_pin)                                                               \
        {                                                                                          \
            if (!gpio_is_ready_dt(&ctx->data_ready_gpio))                                          \
            {                                                                                      \
                LOG_ERR("Data-ready GPIO not ready (inst %d)", inst);                              \
                return -ENODEV;                                                                    \
            }                                                                                      \
            int err = gpio_pin_configure_dt(&ctx->data_ready_gpio, GPIO_INPUT);                    \
            if (err != 0)                                                                          \
            {                                                                                      \
                LOG_ERR("Failed to configure data-ready GPIO (inst %d): %d", inst, err);           \
                return err;                                                                        \
            }                                                                                      \
            err = gpio_pin_interrupt_configure_dt(&ctx->data_ready_gpio, GPIO_INT_EDGE_TO_ACTIVE); \
            if (err != 0)                                                                          \
            {                                                                                      \
                LOG_ERR("Failed to configure data-ready interrupt (inst %d): %d", inst, err);      \
                return err;                                                                        \
            }                                                                                      \
            ...
            if (err != 0)                                                                          \
            {                                                                                      \
                LOG_ERR("Failed to add data-ready callback (inst %d): %d", inst, err);             \
                return err;                                                                        \
            }                                                                                      \
        }                                                                                          \
    } while (0)

#define SPI_BROKER_INIT_INSTANCE(inst)                            \
    do                                                            \
    {                                                             \
        struct spi_broker_ctx *ctx = &spi_broker_ctx_##inst;      \
        if (!spi_is_ready_dt(&ctx->spi))                          \
        {                                                         \
            LOG_ERR("SPI device not ready (inst %d)", inst);      \
            return -ENODEV;                                       \
        }                                                         \
        SPI_BROKER_INIT_GPIO(inst);                               \
        k_work_init_delayable(&ctx->work, exchange_work_handler); \
        ctx->broker = pouch_serial_broker_create(&ctx->adapter);  \
        if (ctx->broker == NULL)                                  \
        {                                                         \
            LOG_ERR("Failed to create broker (inst %d)", inst);   \
            return -ENOMEM;                                       \
        }                                                         \
        LOG_DBG("Initialized SPI broker (inst %d)", inst);        \
    } while (0)
...
#define START(inst)                                          \
    ...
    DT_INST_FOREACH_STATUS_OKAY(START);
}
