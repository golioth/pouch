...
static const struct bt_uuid_128 char_uuids[BROKER_BT_ATTRS] = {
    [BROKER_BT_ATTR_INFO] = BT_UUID_INIT_128(POUCH_GATT_UUID_INFO_CHRC_VAL),
    [BROKER_BT_ATTR_DOWNLINK] = BT_UUID_INIT_128(POUCH_GATT_UUID_DOWNLINK_CHRC_VAL),
    [BROKER_BT_ATTR_UPLINK] = BT_UUID_INIT_128(POUCH_GATT_UUID_UPLINK_CHRC_VAL),
    [BROKER_BT_ATTR_SERVER_CERT] = BT_UUID_INIT_128(POUCH_GATT_UUID_SERVER_CERT_CHRC_VAL),
    [BROKER_BT_ATTR_DEVICE_CERT] = BT_UUID_INIT_128(POUCH_GATT_UUID_DEVICE_CERT_CHRC_VAL),
};
static const struct bt_uuid_16 gatt_ccc_uuid = BT_UUID_INIT_16(BT_UUID_GATT_CCC_VAL);

BUILD_ASSERT(ARRAY_SIZE(char_uuids) == BROKER_BT_ATTRS, "Missing characteristic UUID definitions");
                                    ...
                                    struct bt_gatt_discover_params *params)
{
    ...
    if (attr)
    {
        ...
        for (int i = 0; i < BROKER_BT_ATTRS; i++)
        {
            if (device->chars[i].handle.value < attr->handle)
            {
                if (attr_idx == BROKER_BT_ATTRS)
                {
                    attr_idx = i;
                }
                                        ...
                                        struct bt_gatt_discover_params *params)
{
    ...
    if (!device->chars[BROKER_BT_ATTR_UPLINK].handle.value
        || !device->chars[BROKER_BT_ATTR_DOWNLINK].handle.value)
    {
