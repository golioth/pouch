...
static void finish(struct broker_bt_gatt_device *device)
{
                         ...
                         uint16_t length)
{
    ...
    if (NULL == data)
    {
        ...
        close(c);
        c->callback(device, (enum broker_bt_attr)(c - device->chars));
...
static void link_complete(struct broker_bt_gatt_device *device, enum broker_bt_attr attr)
{
    if (device->chars[BROKER_BT_ATTR_DOWNLINK].subscribed
        || device->chars[BROKER_BT_ATTR_UPLINK].subscribed)
    {
