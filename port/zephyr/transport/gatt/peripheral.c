...
#include "transport/topics/topics.h"
...
static int trx_send(struct pouch_trx *trx, const uint8_t *buf, size_t len)
{
    struct pouch_characteristic *c = CONTAINER_OF(trx, struct pouch_characteristic, trx);
...
static void trx_abort(struct pouch_trx *trx)
{
    struct pouch_characteristic *c = CONTAINER_OF(trx, struct pouch_characteristic, trx);
...
static int open(struct pouch_characteristic *c)
{
    ...
    {
        case CHAR_RECEIVER:
            return pouch_receiver_open(c->receiver, &c->trx);
        case CHAR_SENDER:
            return pouch_sender_open(c->sender, &c->trx);
    }
...
static ssize_t ccc_write(struct bt_conn *conn, const struct bt_gatt_attr *ccc_attr, uint16_t value)
{
    ...
    c->conn = conn;
    c->attr = attr;
    c->trx.maxlen = mtu - BT_ATT_OVERHEAD;
...
static struct pouch_characteristic server_cert = {
    .trx = TRX_INIT,
    .type = CHAR_RECEIVER,
    .receiver = &pouch_topic_server_cert,
};
static struct pouch_characteristic device_cert = {
    .trx = TRX_INIT,
    .type = CHAR_SENDER,
    .sender = &pouch_topic_device_cert,
};
#endif
