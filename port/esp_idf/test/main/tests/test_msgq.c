...
void test_msgq_send_and_receive_basic(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, pouch_msgq_send(&msgq, &val_in, 10));
    TEST_ASSERT_EQUAL_INT(0, pouch_msgq_rcv(&msgq, &val_out, 10));
...
void test_msgq_returns_eagain_on_full(void)
{
    ...
    {
        vals[i] = i;
        TEST_ASSERT_EQUAL_INT(0, pouch_msgq_send(&msgq, &vals[i], 0));
    }
    int extra = 99;
    TEST_ASSERT_EQUAL_INT(-EAGAIN, pouch_msgq_send(&msgq, &extra, 0));
}
...
void test_msgq_returns_enomsg_on_empty(void)
{
    ...
    int val_out = 0;
    TEST_ASSERT_EQUAL_INT(-ENOMSG, pouch_msgq_rcv(&msgq, &val_out, 0));
}
...
void test_msgq_fifo_order(void)
{
    ...
    for (int i = 0; i < MSGQ_SIZE; ++i)
    {
        TEST_ASSERT_EQUAL_INT(0, pouch_msgq_send(&msgq, &vals_in[i], 10));
    }
    for (int i = 0; i < MSGQ_SIZE; ++i)
    {
        TEST_ASSERT_EQUAL_INT(0, pouch_msgq_rcv(&msgq, &val_out, 10));
