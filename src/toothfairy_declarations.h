...
typedef int (*tf_chrc_init_func)(const struct toothfairy_peripheral *, struct bt_gatt_attr *);

struct toothfairy_characteristic
{
    struct bt_gatt_attr *attr;
    tf_chrc_init_func init;
};

#define TOOTHFAIRY_CHARACTERISTIC(name, _uuid, _props, _perm, _init_fn, _read, _write, _user_data) \
    ...
    const STRUCT_SECTION_ITERABLE(toothfairy_characteristic, _##name##_init) = {                   \
        .attr = &name##_val,                                                                       \
        .init = _init_fn,                                                                          \
    }

#define TOOTHFAIRY_SERVICE(_uuid) \
    STRUCT_SECTION_ITERABLE(bt_gatt_attr, AAA_tf_svc) = BT_GATT_PRIMARY_SERVICE(_uuid)
