 ...
 * sent the same message since our last power cycle. This isn't proper replay protection, as it'll
 * get wiped when this device power cycles, but it's a cheap guard against low effort replay
 * attacks.
 */
static uint64_t server_seqnum;
...
static bool is_valid_downlink(const struct session_id *id, psa_algorithm_t algorithm)
{
    ...
    if (session_id_is_equal(&downlink.id, id))
    {
        ...
        return downlink.algorithm != algorithm;
    }
    ...
    if (id->initiator == POUCH_ROLE_SERVER)
    {
        ...
        // number.
        if (id->type == SESSION_ID_TYPE_SEQUENTIAL && id->value.sequential.seqnum <= server_seqnum)
        {
            return false;
        }
    }
                           ...
                           psa_key_id_t private_key)
{
    ...
    if (id->initiator == POUCH_ROLE_DEVICE)
    {
        ...
        {
            // The server claims to use our uplink's session, but it doesn't
            return -EBADMSG;
        }
...
struct pouch_buf *downlink_block_decrypt(struct pouch_buf *block)
{
    ...
    /* As we were able to decrypt a block, we know it's a legitimate session. We can now safely
     * update the server's sequence number.
     */
    if (downlink.id.initiator == POUCH_ROLE_SERVER
        && downlink.id.type == SESSION_ID_TYPE_SEQUENTIAL)
    {
        server_seqnum = downlink.id.value.sequential.seqnum;
    }
