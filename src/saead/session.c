...
void session_init(struct session *session, psa_algorithm_t algorithm)
{
...
int session_id_generate(session_id_t *id)
{
    psa_status_t status;
...
struct pouch_buf *session_encrypt_block(struct session *session, struct pouch_buf *block)
{
    ...
    if (plaintext_len <= AUTH_TAG_LEN || plaintext_len != pouch_bufview_available(&plaintext))
    {
