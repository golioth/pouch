...
LOG_MODULE_REGISTER(pouch_http, CONFIG_POUCH_HTTP_LOG_LEVEL);
...
static int pouch_http_set_sockopt_tls(int sock,
...
int pouch_http_init(const sec_tag_t *sec_tag_list, size_t sec_tag_count)
{
    ...
    ret =
        zsock_getaddrinfo(CONFIG_POUCH_HTTP_GW_URI, CONFIG_POUCH_HTTP_GW_PORT, &hints, &addrs);
