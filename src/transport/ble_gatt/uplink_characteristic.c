...
static void send_indication(struct golioth_ble_gatt_uplink_ctx *ctx)
{
    ...
    enum golioth_ble_gatt_packetizer_result ret =
        golioth_ble_gatt_packetizer_get(ctx->packetizer, (void *) ctx->indicate_params.data, &buf_len);
...
static void uplink_ccc_changed(const struct bt_gatt_attr *attr, uint16_t value)
{
    ...
    else
    {
        ...
        /* TODO: Indications turned off, stop sending */
    }
}

GOLIOTH_BLE_GATT_CCC(uplink, uplink_ccc_changed, uplink_ccc_write, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE);
