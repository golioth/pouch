...
static void send_ack(struct k_work *work)
{
    ...
    int err = pouch_trx_send(p->trx, buf, sizeof(buf));
    if (err)
    {
        // try again later:
...
int pouch_receiver_open(struct pouch_receiver *recv, struct pouch_trx *trx)
{
    ...
    if (recv->handler->start != NULL)
    {
        int err = recv->handler->start(recv);
        if (err)
        {
...
int pouch_receiver_recv(struct pouch_receiver *recv, const uint8_t *buf, size_t len)
{
    ...
    err = recv->handler->recv(recv, pkt.data, pkt.len);
    if (err)
    {
