...
static enum pouch_gatt_packetizer_result device_cert_fill_cb(void *dst,
                                                             size_t *dst_len,
                                                             void *user_arg)
{
                                ...
                                uint16_t offset)
{
    ...
    struct pouch_gatt_device_cert_ctx *ctx = attr->user_data;
    ...
    if (NULL == ctx->packetizer)
    {
        ...
        ctx->packetizer = pouch_gatt_packetizer_start_callback(device_cert_fill_cb, &ctx->cert);
    }
