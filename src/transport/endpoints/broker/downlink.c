...
POUCH_LOG_REGISTER(downlink_endpoint, CONFIG_POUCH_GATEWAY_LOG_LEVEL);
...
static int start(struct pouch_bearer *bearer)
{
    ...
    {
        POUCH_LOG_ERR("Failed to open downlink");
        return -ENOMEM;
    }
...
static enum pouch_result send(struct pouch_bearer *bearer, void *dst, size_t *dst_len)
{
    ...
    if (err)
    {
        POUCH_LOG_ERR("Error getting downlink data: %d", err);
        return POUCH_ERROR;
    }
