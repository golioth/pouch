...
static int settings_decode(zcbor_state_t *zsd, void *value)
{
    ...
    while (!zcbor_list_or_map_end(zsd))
    {
        ...
        switch (major_type)
        {
            ...
            {
                value.type = SETTING_VALUE_TYPE_INT;
            ...
            case ZCBOR_MAJOR_TYPE_SIMPLE:
            {
                ...
                {
                    value.type = SETTING_VALUE_TYPE_FLOAT;
                }
                else if (zcbor_bool_decode(zsd, &value.bool_val))
                {
                    value.type = SETTING_VALUE_TYPE_BOOL;
                }
                else
                {
                    data_type_valid = false;
...
static void settings_uplink(void)
{
    ...
    pouch_uplink_entry_write(".c/status",
...
GOLIOTH_SERVICE(settings, "/.c", settings_downlink, settings_uplink);
