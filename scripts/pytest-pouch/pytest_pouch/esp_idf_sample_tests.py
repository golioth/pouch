...
def pouch_test_boot_banner_pattern() -> str:
    return r".*Pouch .* Example"


@pytest.fixture(scope="module")
def pouch_test_boot_state_pattern():
    return re.compile(
        r".*(Failed to load credentials|"
...
@pytest.fixture(scope="module")
def pouch_test_store_result_pattern() -> str:
    return (
        r".*(Successfully stored .+|Failed to store .+|"
        r"Failed to set .+|Failed to decode base64.*)"
    )
...
async def _send_and_expect_store(
    dut, command: str, expected_log: str, store_result_pattern: str
) -> None:
    await _dut_write(dut, command)
    match = await _dut_expect(
        dut,
        store_result_pattern,
...
async def _detect_boot_state(
    dut,
    banner_timeout_s: float,
    boot_banner_pattern: str,
    boot_state_pattern,
) -> bool:
    """Return True when boot logs indicate credentials must be provisioned."""

    await _dut_expect(dut, boot_banner_pattern, timeout=banner_timeout_s)
...
async def _wait_for_boot_state(
    dut, boot_banner_pattern: str, boot_state_pattern
) -> bool:
    ...
    for timeout_s, do_reset in (
        ...
        try:
            return await _detect_boot_state(
                dut,
                timeout_s,
                boot_banner_pattern,
                boot_state_pattern,
...
async def _provision_and_boot(
    dut,
    provisioning_creds,
    provision_wifi: bool,
    store_result_pattern: str,
    boot_banner_pattern: str,
) -> None:
    ...
    if provision_wifi:
        ...
        print("Provisioning credentials")
    ...
    await _dut_expect(dut, boot_banner_pattern, timeout=REBOOT_TIMEOUT_S)
    await _dut_expect(
...
async def connected_cloud_session(
    cloud_config,
    provisioning_creds,
    dut,
    project,
    pouch_test_provision_wifi,
    pouch_test_store_result_pattern,
    pouch_test_boot_banner_pattern,
    pouch_test_boot_state_pattern,
):
    needs_provisioning = await _wait_for_boot_state(
        dut,
        pouch_test_boot_banner_pattern,
        pouch_test_boot_state_pattern,
    )
...
async def test_ota_firmware_update(
    ...
    await _dut_expect(
        dut, r".*Received LED setting: [01]", timeout=OTA_REBOOT_TIMEOUT_S
    )
