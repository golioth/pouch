...
#define COAP_CA_FILE CERT_DIR "/ca.der"
#define GW_CERT_FILE CERT_DIR "/" CONFIG_EXAMPLE_COAP_CLIENT_GW_DEVICE_CRT_FILENAME
#define GW_KEY_FILE CERT_DIR "/" CONFIG_EXAMPLE_COAP_CLIENT_GW_DEVICE_KEY_FILENAME
...
#ifndef CONFIG_EXAMPLE_COAP_CLIENT_DTLS_LOAD_CA_FROM_FILESYSTEM
static const uint8_t dtls_ca_crt[] = {
...
{
    /* Ensure the credentials directory exists, so the user does not need to create it manually. */
...
static ssize_t load_file_to_tls(const char *filename,
                                sec_tag_t sec_tag,
...
{
#ifdef CONFIG_EXAMPLE_COAP_CLIENT_DTLS_LOAD_CA_FROM_FILESYSTEM
    if (NULL != _m_dtls_ctx.ca_cert)
    {
        LOG_ERR("Free existing CA cert before loading");
        return -EEXIST;
    }

    _m_dtls_ctx.ca_cert_len = load_file_to_tls(COAP_CA_FILE,
                                               sec_tag,
                                               TLS_CREDENTIAL_CA_CERTIFICATE,
                                               &_m_dtls_ctx.ca_cert);
    return _m_dtls_ctx.ca_cert_len;
#else
...
{
    if (NULL != _m_dtls_ctx.gw_crt)
    {
        LOG_ERR("Free existing gateway cert before loading");
        return -EEXIST;
    }

    _m_dtls_ctx.gw_crt_len = load_file_to_tls(GW_CERT_FILE,
                                              sec_tag,
                                              TLS_CREDENTIAL_PUBLIC_CERTIFICATE,
                                              &_m_dtls_ctx.gw_crt);
    return _m_dtls_ctx.gw_crt_len;
}

int load_coap_gw_device_key(sec_tag_t sec_tag)
{
    if (NULL != _m_dtls_ctx.gw_key)
    {
        LOG_ERR("Free existing gateway key before loading");
        return -EEXIST;
    }

    _m_dtls_ctx.gw_key_len =
        load_file_to_tls(GW_KEY_FILE, sec_tag, TLS_CREDENTIAL_PRIVATE_KEY, &_m_dtls_ctx.gw_key);
    return _m_dtls_ctx.gw_key_len;
}

void free_coap_certs(void)
{
    if (IS_ENABLED(CONFIG_EXAMPLE_COAP_CLIENT_DTLS_LOAD_CA_FROM_FILESYSTEM))
    {
        free(_m_dtls_ctx.ca_cert);
    }

    _m_dtls_ctx.ca_cert = NULL;
    _m_dtls_ctx.ca_cert_len = 0;

    free(_m_dtls_ctx.gw_crt);
    _m_dtls_ctx.gw_crt = NULL;
    _m_dtls_ctx.gw_crt_len = 0;

    free(_m_dtls_ctx.gw_key);
    _m_dtls_ctx.gw_key = NULL;
    _m_dtls_ctx.gw_key_len = 0;
}
