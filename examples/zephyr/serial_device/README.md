# Pouch SPI Serial Device Example

The Pouch SPI Serial Device example demonstrates how to create a Pouch
application based on the SPI serial transport.

Unlike the BLE GATT transport, where the device advertises over Bluetooth and
waits for a gateway to initiate a connection, the SPI serial transport
exposes the device as an SPI peripheral that a directly-connected Pouch
gateway polls over a dedicated SPI bus. There is no discovery or connection
management: the gateway and device are wired together and communicate as
soon as both are running.
...
primarily developed and tested on the `nrf52840dk/nrf52840` board, but any
Zephyr board with an SPI peripheral, PSA, MbedTLS and LittleFS support
should work.

The SPI peripheral is configured through a board-specific devicetree overlay (see
`nrf52840dk_nrf52840.overlay` for reference). The overlay declares a
`pouch-serial` child node under the SPI controller that binds to the
`golioth,pouch-device` compatible, providing the CS, data-ready and
`spi-max-frequency` properties used by the transport.

## Authentication

The Pouch SPI Serial Device example requires a private key and certificate to
...
direct to the Golioth. Please refer to [the official
