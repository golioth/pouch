# Pouch CoAP Client Example

The Pouch CoAP client example demonstrates how to create a Pouch application
based on the CoAP/DTLS client transport.
...
board with CoAP, PSA, MbedTLS and LittleFS support should work.
...
The Pouch CoAP client example requires a private key and certificate to
...
Golioth uses Mutual DTLS (mDTLS) for the CoAP client transport. By default, the
Pouch CoAP client example will use the same device credentials for mDTLS as it
